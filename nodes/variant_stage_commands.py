...
REPORT_SCRIPT = _REPO_ROOT / "engine" / "src" / "bioflow" / "runtime" / "variant_report.py"
...
def bcftools_stats_argv(vcf_path, extra_command="") -> list[str]:
    return conda_command(ENV_NAME, "bcftools", "stats", str(vcf_path), *parse_extra_command_tokens(extra_command))
