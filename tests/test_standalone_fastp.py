...
def test_fastp_galaxy_options_and_extra_command_are_optional(fastp_module):
    inputs = fastp_module.Fastp.INPUT_TYPES()
    ...
    assert set(fastp_module.NODE_CLASS_MAPPINGS) == {"Fastp"}
...
def test_fastp_reports_missing_binary_before_creating_output(fastp_module, tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="fastp executable not found on PATH"):
        fastp_module.Fastp().run(str(read1), str(output_dir))
...
def test_fastp_runs_on_upstream_paired_data(fastp_module, tmp_path, monkeypatch):
    ...
    out1, out2, report_json, report_html = fastp_module.Fastp().run(
        str(r1),
        "out",
        str(r2),
        threads=2,
