...
def test_quast_run_signature_has_no_var_keyword(quast_module):
    sig = inspect.signature(quast_module.QuastNode.run)
...
def test_quast_galaxy_options_and_extra_command_are_optional(quast_module):
    inputs = quast_module.QuastNode.INPUT_TYPES()
    assert set(inputs["required"]) == {"assembly_fasta", "output_dir"}
    ...
    assert set(quast_module.NODE_CLASS_MAPPINGS) == {"QuastNode"}
    assert set(quast_module.NODE_DISPLAY_NAME_MAPPINGS) == {"QuastNode"}


def test_quast_removes_managed_options_but_keeps_native_extra(quast_module):
    kept, ignored = quast_module._filter_extra(
        "-m1000 -t8 -r ref.fa -g feat.gff --min-contig=500 --threads=4 --features=feat2.gff --large --silent --fast",
        quast_module._MANAGED_OPTIONS,
    )
    assert kept == ["--silent", "--fast"]
    assert "-m1000" in ignored
    assert "-t8" in ignored
    assert "-r" in ignored
    assert "-g" in ignored
    assert "--min-contig=500" in ignored
    assert "--threads=4" in ignored
    assert "--features=feat2.gff" in ignored
    assert "--large" in ignored
...
def test_quast_reports_missing_binary_before_creating_output(quast_module, tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="quast.py executable not found on PATH"):
        quast_module.QuastNode().run(str(assembly), str(output_dir))
...
def test_quast_reports_missing_reference_before_creating_output(quast_module, tmp_path):
    ...
    output_dir = tmp_path / "absent"
    with pytest.raises(FileNotFoundError, match="is not a file"):
        quast_module.QuastNode().run(str(assembly), str(output_dir), reference=str(reference))
    assert not output_dir.exists()
...
def test_quast_runs_on_spades_assembled_contigs(quast_module, tmp_path, monkeypatch):
    ...
    contigs, scaffolds, log = spades.SpadesNode().run(
        str(r1),
        "spades_out",
        read2=str(r2),
        threads=2,
    ...
    report_tsv, report_html = quast_module.QuastNode().run(
        contigs,
        "quast_out",
        min_contig=100,
        threads=2,
