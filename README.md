...
- **2개 노드 실행 계층 (총 98개 활성 등록 노드)**:
  - **Class 1 (40개 노드)**: ComfyUI 인메모리 고속 과학 연산(Biopython, Scanpy, AnnData, tximport) 및 20종의 웹 캔버스 연동 시각화 노드(`IMAGE` 텐서 직결).
...
- **입력 반응형 캐시 무효화 (`IS_CHANGED`)**: 파일 경로, 파일 크기(`st_size`), 최종 수정 시각(`st_mtime_ns`) 및 파라미터 해싱 기반의 결정론적 캐시 키를 생성하여, 파라미터 또는 파일 변경 시 영향받는 서브그래프만 선택적으로 재실행합니다 (캐시 판정 지연 **0.15 ms** 이내).
...
## 2. 노드 분류 및 워크플로우 검증 현황

재현성 검증을 위한 핵심 워크플로우는 `workflows/` 디렉터리에 워크플로우 메타데이터가 자체 내장된 ComfyUI 워크플로우 PNG 이미지(`final_01_pancreas_scanpy_e2e.png`, `final_02_zymo_metaphlan_e2e.png`, `final_03_gse110004_chri_smoke.png`) 및 JSON 파일로 제공됩니다. ComfyUI 웹 인터페이스에서 해당 PNG 파일을 캔버스 위로 드래그 앤 드롭하면 전체 워크플로우 그래프가 즉시 복원되어 웹상에서 결과를 확인할 수 있습니다.

전체 98개 등록 노드의 분류, 실행 환경, 검증 등급(L2 7개 / L1+S 55개 / L1 36개) 및 입출력 명세는 다음 자료에 공개되어 있습니다:
...
- 노드 검증 매트릭스, 사례 연구 실험 프로토콜(입력 SHA-256 명세 포함), 독립 재현성 가이드는 원고와 함께 제출되는 보충 파일로 제공됩니다.
...
```bash
# 1. 실행 계약, 환경 격리, 캐시 무효화 및 산출물 무결성 회귀 검증
pytest -v tests/test_execution_contract.py

# 2. ComfyUI 그래프 위상 정렬 및 DAG 라이프사이클 검증
pytest -v tests/test_comfyui_graph_execution.py

# 3. 원 도구(Native CLI / Bioconductor)와의 수치 동등성 E2E 검증
pytest -v -m e2e tests/test_native_concordance.py

# 4. 비교기 결함 주입 차단 검증 (10종 결함 전수 통과)
pytest -v tests/test_comparator_fault_injection.py

# 5. ComfyUI 노드 등록, 포트 서명 및 그래프 무결성 통합 검증
pytest -v tests/test_comfyui_integration.py

# 6. 기본 자동화 테스트 스위트(10개 모듈 63개 테스트) 전수 실행
pytest -v
```
